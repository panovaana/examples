...
IF OBJECT_ID('ISSUANCE') IS NOT NULL
	DROP VIEW ISSUANCE;
GO 
...
FROM(
	...
	JOIN
		READER
		ON READER.reader_id = ISSUE.reader_id) AS view_1
JOIN (
	SELECT 
		book_id AS book_id
		,(SELECT theme_nm + ' ' 
	...
	FROM 
		(SELECT BOOK_BY_THEME.book_id AS book_id, THEME.theme_nm AS theme_nm
			...
			GROUP BY book_id) AS view_2
		ON view_1.book_id = view_2.book_id
)
GO
SELECT * FROM ISSUANCE;
GO
--Топ 3 задолжностей читателей библиотеки
SELECT TOP(3) WITH TIES
	ISSUANCE.reader_surname
	, ISSUANCE.overdue
FROM
	ISSUANCE
ORDER BY ISSUANCE.overdue DESC
